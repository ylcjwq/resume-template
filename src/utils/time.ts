 ...
 * @returns {string} date
 */
export const intToDateString = (num: number, unit = '/'): string => {
  let date;
  if (!num) {
    date = new Date();
  } else {
 ...
 * @returns date
 */
export const intToTimeString = (num?: number): string => {
  let date;
  if (!num) {
    date = new Date();
  } else {
 ...
 * @param unit 分隔符
 */
export const formatToString = (num: number | undefined, unit = '/') => {
  if (!num) return '';
